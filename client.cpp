...
void *worker_function(BoundedBuffer* buf, FIFORequestChannel* chan, string fileToGet, HistogramCollection* collect /*pointer to histcollection*/)
{
    //ofstream retrieved;
    ...
    */
    ...
    fileToGet = fileToGet;
     ...
     string strFile = "/home/sneal/PA3/received/" + fileToGet;
...
void *file_function(BoundedBuffer* buf, FIFORequestChannel* chan, int b, string fileNameInput)
{
    //recieving seg fault 
...
int main(int argc, char *argv[])
{
    ...
    vector<FIFORequestChannel*> FIFO_Fun;
    mutex mu;
    ...
    for(int y = 0; y < w; y++)
    {
        ...
        FIFO_Fun.push_back(new FIFORequestChannel(name, FIFORequestChannel::CLIENT_SIDE));
    }
    ...
    if(fileTime)
    {
        threadFile.push_back(move(new thread(file_function, &request_buffer, chan, b, fileToGet)));////////////////////
    }
    
    //cout << "rrrrrrrrrr" << endl;
    for(int z = 0; z < w; z++)
    {
        threadWork.push_back(move(new thread(worker_function, &request_buffer, FIFO_Fun[z], fileToGet, &hc )));
    }
